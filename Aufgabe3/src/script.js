 ...
 */
 ...
 function main(point, pointcloud) {
   ...
   bushaltestellen.abfahrten(resultarray[1].id, 1800);
   bushaltestellen.abfahrten(resultarray[0].id, 1800);
   Tablestructure.clearTable('secondTable');
   Tablestructure.drawSecondTable(resultarray);
   Tablestructure.firstTableHeader(resultarray[0].name, resultarray[1].name);
 }
 ...
 class BusRadarAPI {
   ...
   abfahrten(id, time) {
     ...
     x.onload = () => {
       if (x.status >= 400) {
         ...
         Tablestructure.drawFirstTable(depatures);
       }
     };
     ...
     x.send();
     Tablestructure.clearTable("firstTable");
   }
 
   /**
    * secondsToTime
    * @desc converts the given time in secons by the API to ISO 8601
   ...
   static secondsToTime(seconds) {
     ...
     var ms = seconds * 1000;
     var time = new Date(ms).toISOString().slice(0, -5);
     return time + " GMT";
   }
 }
 ...
 class Tablestructure {
   ...
   /**
    * firstTableHeader
    * @desc creates and updates the header for the first table, which contains information
    * about which is the next, and second next bus stop for comparing the direction to the 
    * direction of the second table so the user knows which stop goes in which direction
    * @param {*} nextBusStop
    * @param {*} secondNextBusStop
    */
   static firstTableHeader(nextBusStop, secondNextBusStop) {
     // for the case there is no upcoming depature from the closest busstop
    if (secondNextBusStop === undefined) {
       let message = "departures from " + nextBusStop + " in the next 30 minutes";
       document.getElementById("firstTableHeader").innerHTML = message;
     }
     // for the case there are upcoming depatures in the next 30 minutes from the closest two bus stops
     else {
   ...
   /**
    * drawFirstTable
    * @desc inserts the resultarray into the Table on the web-page
    * @param {*} resultarray array of JSON with contains
    */
   static drawFirstTable(resultarray) {
     ...
     // for the case there are no depatures from the closest two bus stops
    ...
    * @param {*} resultarray array of JSON with contains
    */
   static drawSecondTable(resultarray) {
 ...
 /** 
  * Class for calculating distances and sorting the 
  */
