...
meta.content = "This website is for calculating points from a choosen point to given pois";
document.head.appendChild(meta);
...
// allocating the stored location to an id for DOM
...
function getFile() {
    ...
    if (isValid(reader.result) == true) { // Checks whether the input is valid
        if ((JSON.parse(reader.result)).type != "Point") {
            document.getElementById("errorMessage2").innerHTML = 'ERROR: This is not a Point. Expected pattern: {"type":"Point","coordinates":[...]}'
        } else {
